		...
		<div class="bigor">
			...
			<?php if($serieslatest){
				...
				foreach ($serieslatest as $key => $value) {
					echo '<li><a href="'.chapter_url($value['permalink']).'"><span class="fivchap">'.GOV_lang::get('mini_ch').' '.$value['chapter'].'</span><span class="fivtime">'.human_time_diff($value['time'], current_time('timestamp')).'</span></a></li>';
				}
				echo '</ul>';
			} ?>
